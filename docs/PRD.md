...
> 我从微信转了 500 到银行卡 → 记账 Tab → 选"转账" → 选转出"微信" → 选转入"银行卡" → 金额 500 → 保存。
> 微信账户 -500，银行卡账户 +500，本月总支出 **不变**（转账不算支出）。
...
| F6 | 统计概览 | 本月已花 / 收入 / 结余 + 分类饼图 + 支出占收入比例条 | "账单" Tab 顶部；可切换月份；本月收入为 0 时不显示比例条 |
...
2. 数据库只插**一条**记录，`type = TRANSFER`，`amount` 为正数
3. **转出账户 ≠ 转入账户**；相同则禁止保存并提示
4. 转账**不**影响本月"支出"和"收入"统计，但会影响：
   - 转出账户余额（-amount）
   - 转入账户余额（+amount）
5. 转账记录在列表里显示「微信 → 银行卡  ¥500」
6. 余额计算（运行时，**不落库存余额字段**）：
   ```
   账户余额 = initial_balance
            + 该账户作为 account_id 的 INCOME 总额
            - 该账户作为 account_id 的 EXPENSE 总额
            - 该账户作为 account_id 的 TRANSFER 总额
            + 该账户作为 to_account_id 的 TRANSFER 总额
   ```
